...
function App() {
    ...
    return (<div className="min-h-screen p-8">
      ...
      <div className="grid gap-6">
        {runs.map(run => (<div key={run.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-start justify-between">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${run.status === 'passed' ? 'bg-green-100 text-green-800' :
            ...
            {run.testCases && run.testCases[0]?.bugReports?.length > 0 && (<div className="mt-4 p-4 bg-red-50 rounded-lg border border-red-100">
                <h3 className="font-bold text-red-900 mb-2">Bug Report</h3>
                {run.testCases[0].bugReports.map((bug) => (<div key={bug.id} className="text-sm">
        ...
        {runs.length === 0 && (<div className="text-center py-12 text-gray-500">
